% Load real test data
filename = 'Atlanta_Shif_TypSum_RB_2004_TypOcc_TypBehav_TES_08092022_074831.mat';
old = load(filename);
...
ntimestep=T/60+240; % total number of time steps; ice tank is charged for 240 minutes before midnight
% Initialize storage arrays
MyCtrlk = zeros(ntimestep+1,29);
ZoneInfoK = zeros(ntimestep+1,32);
for timestep=-239:ntimestep
    %% At every iteration,update measurements
    % The ice tank is charge starting at 20:01 the day before, so the
    % timestep starts negative, but the MATLAB index starts at 1, so
    % introduce a MATLAB index
    timestep_idx = timestep+239; 
    timestep
    HardwareTime = table2array(inputs(timestep_idx+1,1)); %0.0001; % Please assign index number of the hardware data to this variable
    Meas = table2array(inputs(timestep_idx+1,3:end));
    if timestep > 0
    ...
    MyCtrlk(timestep+1+239,:) = MyCtrl;
    ZoneInfoK(timestep+1+239,:) = ZoneInfo;
end
...
figure(1)
subplot(2,2,1)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.scaledData2.ahu1_fan_power), hold on
plot(x,MyCtrlk(:,14)), hold off
legend('Data','Sim')
title('AHU1 Power')
subplot(2,2,2)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.scaledData2.ahu2_fan_power), hold on
plot(x,MyCtrlk(:,13)), hold off
legend('Data','Sim')
title('AHU2 Power')
subplot(2,2,3)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.processData2.ahu1_fan_sp_inh2o), hold on
plot(x,ctrlTable.P_sp_ahu1*0.00401865), hold off
legend('Data','Sim')
title('P_{sp_ahu1}')
subplot(2,2,4)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.processData2.ahu2_fan_sp_inh2o), hold on
...
figure(2)
subplot(2,2,1)
plot([A.SimData.T_z1_ahu1]-[A.SimData.Tz_cspt_z1_ahu1]), hold on
plot(x,zoneTable.T_z1_ahu1-zoneTable.Tz_cspt_z1_ahu1), hold off
legend('data','sim')
title('3')
subplot(2,2,2)
plot([A.SimData.T_z2_ahu1]-[A.SimData.Tz_cspt_z2_ahu1]), hold on
plot(x,zoneTable.T_z2_ahu1-zoneTable.Tz_cspt_z2_ahu1), hold off
legend('data','sim')
title('4')
subplot(2,2,3)
plot([A.SimData.T_z1_ahu2]-[A.SimData.Tz_cspt_z1_ahu2]), hold on
plot(x,zoneTable.T_z1_ahu2-zoneTable.Tz_cspt_z1_ahu2), hold off
legend('data','sim')
title('1')
subplot(2,2,4)
plot([A.SimData.T_z2_ahu2]-[A.SimData.Tz_cspt_z2_ahu2]), hold on
...
figure(3)
subplot(2,2,1)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.processData2.vav1_d_sp), hold on
plot(x,MyCtrlk(:,1)), hold off
legend('data','sim')
grid on
title('VAV1')
subplot(2,2,2)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.processData2.vav2_d_sp), hold on
plot(x,MyCtrlk(:,2)), hold off
legend('data','sim')
grid on
title('VAV2')
subplot(2,2,3)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.processData2.vav3_d_sp), hold on
plot(x,MyCtrlk(:,3)), hold off
legend('data','sim')
grid on
title('VAV3')
subplot(2,2,4)
plot(A.HardwareData.processData2.comms_timestep,A.HardwareData.processData2.vav4_d_sp), hold on
